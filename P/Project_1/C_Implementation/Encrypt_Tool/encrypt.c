#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <openssl/sha.h>

void getInput()
{
    char *line = NULL;
    size_t len = 0;
    ssize_t lineSize = 0;
    lineSize = getline(&line, &len, stdin);
    
    //  Number of chars = lineSize -1
    printf("Input has %zu chars.\n", lineSize -1);

    free(line);
}

void getKey(char *filePath, unsigned char* seed)
{
    printf("[!] Using key PATH: %s\n", filePath); // DEGUB
    FILE* fp;
    ssize_t r;
 
    // Opening file in reading mode
    fp = fopen(filePath, "r");
 
    if (NULL == fp) {
        printf("[!] File can't be opened\n");
    }else{
        // Save key
        if(fgets(seed, 33, fp) == NULL){
            printf("[!] Failed to read\n");
        }else{
            printf("[!] Using key: %s\n", seed); // DEGUB

            // Generate seed from key
            SHA256(seed, strlen(seed), seed);

            int i, j = 0;
            char hexStr[65];
            for (i = 0 ; i < 32 ; i++)
            {
                sprintf(hexStr + j, "%02X", seed[i]);
                j += 2;
            }

            hexStr[j] = '\0';

            strcpy(seed, hexStr);

            printf("[!] Using seed: %s\n", seed); // DEGUB
        }

        // Closing the file
        fclose(fp);
    }
}

typedef unsigned char uint8_t;

// Non liniear FSRs
static int getNextInt(int seed)
{
    int n1 = (int) (seed / 10) % 10;
    int n2 = (int) (seed / 100) % 10;
    int n3 = (int) (seed / 1000) % 10;
    int n4 = (int) (seed / 10000) % 10;
    int n5 = (int) (seed / 100000) % 10;

    return ((n1*n3*n4 + n1*n2 + n2*n3 + n3*n4 + n1 + n2) * seed + n5) % 99999;
}

// Fisher-Yates shuffle Algorithm
void shuffle(uint8_t *array, int seed)
{
    int i, j;
    uint8_t tmp;
 
    for (i = 255 ; i > 0; i--) {
        seed = getNextInt(seed);
        j = seed % 256;
        tmp = array[j];
        array[j] = array[i];
        array[i] = tmp;
   }
}

int stringToInt(char seed[])
{
    int result = 0;

    for (int i = 0 ; i < strlen(seed) ; i++){
        result += seed[i] * (i + 1);
    }

    return result % 99999;
}

void generate_Seeds(int seeds[16], int stringInt)
{
    for (int  i = 0 ; i < 16 ; i++){
        seeds[i] = ((i + 2)*stringInt) % 99999;
    }
}

void generate_SBox(uint8_t SBox[16][256], char seed[])
{
    int seeds[16]; 
    
    int stringInt = stringToInt(seed);
    
    generate_Seeds(seeds, stringInt);

    printf("[*] Getting SBoxes\n");

    for (int  i = 0 ; i < 16 ; i++){
        shuffle(SBox[i], seeds[i]);
    }
}

int permutation(uint8_t in[4], uint8_t sbox[256]) 
{
    uint8_t out[4];

    int index = in[0];
    out[3] = sbox[index];

    index = (index + in[1]) % 256;
    out[2] = sbox[index];

    index = (index + in[2]) % 256;
    out[1] = sbox[index];

    index = (index + in[3]) % 256;
    out[0] = sbox[index];
    
    memcpy(in, out, 4);
    return 0;
}

int feistalIter(char firstSlice[4], char secondSlice[4], uint8_t sbox[256]) {
    //printf("firstSlice: %c, secondSlice: %c\n", firstSlice[i], secondSlice[i]); // DEBUG
    
    // Copy array to later switch
    char tmp[4];
    memcpy(tmp, secondSlice, 4);

    // Apply permutation
    permutation(secondSlice, sbox);

    // Apply XOR Li with fi
    for ( int i = 0 ; i < 4 ; i++){
        secondSlice[i] ^= firstSlice[i];
    }

    // Switch position between slices
    memcpy(firstSlice, tmp, 4);

    return 0;
}

void ApplyPermutations(uint8_t firstSlice[4], uint8_t secondSlice[4], uint8_t SBox[16][256])
{
    int NUM_ITERATIONS = 16;

    // Apply permutations
    printf("[*] Permutating\n");
    for( int i = 0 ; i < NUM_ITERATIONS; i += 1 ){
        feistalIter(firstSlice, secondSlice, SBox[i]);
    }
}

void getCipher(uint8_t SBox[16][256])
{
    // Test Cases
    uint8_t firstSlice[4] ={0x01, 0x00, 0x00, 0x00};
    uint8_t secondSlice[4] ={0x00, 0x00, 0x00, 0x00};

    printf("[*] Applying Permutations\n");
    ApplyPermutations(firstSlice, secondSlice, SBox);

    // Print Ciphertext
    printf("[!] Result:");
    for( int i = 0 ; i < 4; i += 1 ){
        printf(" %02X",firstSlice[i]);
    }

    for( int i = 0 ; i < 4; i += 1 ){
        printf(" %02X",secondSlice[i]);
    }
    printf("\n");
}

void getDESCipher()
{
    printf("[!] Using DES\n");
}

int main(int argc, char **argv)
{
    char *keyFile = NULL;
    int deafultDes = 0;
    unsigned char seed[65];

    // Set Path to key file
    if(argc > 2 && strcmp(argv[1],"-k") == 0)
    {
        keyFile = argv[2];
    } else {
        keyFile = "key.txt";
    }

    // Set if algorithm to be used is DES or E-DES(default)
    if(argc == 2 && (strcmp(argv[1],"-d") == 0) || (argc == 4 && strcmp(argv[3],"-d") == 0))
    {
        deafultDes = 1;
    }

    // Retrieve key digest to be used
    getKey(keyFile, seed);

    // Check if algorithm to be used is default or not
    if (deafultDes)
    {
        getDESCipher();
    }else{
        printf("[!] Using E-DES\n");

        uint8_t SBox[16][256] = {
            {},
            {},
            {},
            {},
            {},
            {},
            {},
            {},
            {},
            {},
            {},
            {},
            {},
            {},
            {},
            {}
        };
        
        generate_SBox(SBox, seed);

        getCipher(SBox);
    }

    return 0;
}